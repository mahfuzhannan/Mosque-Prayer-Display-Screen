    ...
    <div className="w-full min-w-[750px]">
      {monthly_prayer_times.map((monthly_prayer_time) => (
        <CalendarPage
          ...
          monthly_prayer_times={monthly_prayer_time} year={year}
          metadata={metadata}/>
      ))}
    </div>
  )
...
function CalendarPage ({ year, monthly_prayer_times, metadata }: {
  year: string,
  ...
  return (
    <div className={cn(
      'print-page page-break a4-portrait flex flex-col justify-start w-full items-center ',
      backgroundColor)}>
      <CalendarHeader
...
function CalendarTable ({ monthly_prayer_times }: {
  monthly_prayer_times: CalendarPrintMonthlyPrayerTimes,
}) {
  const isAsrMithl2 = monthly_prayer_times.prayer_times.some(
    (prayer_time) => prayer_time.asr.start_secondary)
    ...
    <div className="w-full bg-white">
      <table className="w-full table-auto bg-mosqueBrand-primary/20">
        ...
        <tr className="h-8 border-y-2 border-mosqueBrand-primary">
          ...
          <th colSpan={isAsrMithl2 ? 3 : 2}
              className={cn(classNamesFirstHeader)}>Asr
          </th>
        ...
        <tr className="h-6 border-y-2 border-mosqueBrand-primary">
          ...
          {[
            ...
            { label: 'Begins', show: true },
            { label: 'Jamāʿah', show: true },
          ].filter(col => col.show).map((col, idx) => (
            <th key={idx} className={cn(classNamesSecondHeader,
              col.label === 'Jamāʿah' ? 'bg-mosqueBrand-highlight/50' : '')}>
              {col.label}
            </th>
          ))}
        </tr>
...
function CalendarRow ({ prayer_time }: {
  ...
  const defaultClassNames = `text-center text-lg ${isFriday
    ? 'font-semibold border-b-2 border-mosqueBrand-primary'
    : ''}`
  const congregationClassNames = `text-lg text-center bg-mosqueBrand-highlight/50 ${isFriday
    ? 'font-semibold border-b-2 border-mosqueBrand-primary'
    : ''}`
  ...
  return (
    ...
    >
      ...
      <td className={cn(defaultClassNames,
        'border-r-2 border-mosqueBrand-primary')}>{prayer_time.day_of_month}</td>
      <td className={defaultClassNames}>{prayer_time.fajr.start}</td>
      <td
        className={congregationClassNames}>{prayer_time.fajr.congregation_start}</td>
      <td className={cn(defaultClassNames,
        'border-r-2 border-mosqueBrand-primary')}>{prayer_time.sunrise_start}</td>
      <td className={defaultClassNames}>{prayer_time.zuhr.start}</td>
      <td className={cn(congregationClassNames,
        'border-r-2 border-mosqueBrand-primary')}>{prayer_time.zuhr.congregation_start}</td>
      <td className={defaultClassNames}>{prayer_time.asr.start}</td>
      {isAsrMithl2 &&
          <td className={defaultClassNames}>{prayer_time.asr.start_secondary}</td>}
      <td className={cn(congregationClassNames,
        'border-r-2 border-mosqueBrand-primary')}>{prayer_time.asr.congregation_start}</td>
      <td className={defaultClassNames}>{prayer_time.maghrib.start}</td>
      <td className={cn(congregationClassNames,
        'border-r-2 border-mosqueBrand-primary')}>{prayer_time.maghrib.congregation_start}</td>
      <td className={defaultClassNames}>{prayer_time.isha.start}</td>
      <td className={cn(congregationClassNames,
        '')}>{prayer_time.isha.congregation_start}</td>
    </tr>
  )
}
...
function CalendarHeader ({ metadata, year, month }: {
  ...
  return (
    <div className="flex flex-col justify-center items-center w-full gap-2">
      <div
        className="text-mosqueBrand-onPrimary flex flex-row justify-between items-center w-full px-2 pb-2">
        <h2
          className="text-3xl uppercase font-bold text-left flex-1 whitespace-nowrap">{month} {year}</h2>
